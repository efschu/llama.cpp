...
static inline int ggml_cuda_fattn_kv_rank(const ggml_type type) {
    switch (type) {
        ...
        case GGML_TYPE_Q6_0:        return 3;
        case GGML_TYPE_Q5_1:        return 4;
        case GGML_TYPE_Q5_0:        return 5;
        case GGML_TYPE_TURBO4_0:    return 6;
        case GGML_TYPE_Q4_1:        return 7;
        case GGML_TYPE_Q4_0:        return 8;
        case GGML_TYPE_TURBO3_TCQ:  return 9;
        case GGML_TYPE_TURBO3_0:    return 10;
        case GGML_TYPE_TURBO2_TCQ:  return 11;
        case GGML_TYPE_TURBO2_0:    return 12;
        default:                    return -1;
    }
}
...
#undef TURBO_FUSED_DISPATCH
    ...
    if (turbo_prefill_mma_safe && !turbo_prefill_vec && Q->ne[1] > 1 && Q->ne[0] <= 256 && turing_mma_available(ggml_cuda_info().devices[ggml_cuda_get_device()].cc)) {
        ...
        const bool force_vec_for_raw_mixed_kv_pair =
            Q->ne[1] > 1 &&
            (ggml_cuda_fattn_is_turbo_kv_type(K->type) || ggml_cuda_fattn_is_turbo_kv_type(V->type)) &&
            !turbo_prefill_mma_safe;

        const best_fattn_kernel selected_kernel =
            force_vec_for_raw_mixed_kv_pair
...
bool ggml_cuda_flash_attn_ext_supported(int device, const ggml_tensor * dst) {
    ...
    if (kernel == BEST_FATTN_KERNEL_VEC) {
        return ggml_cuda_fattn_pair_compiled(K->type, V->type);
    }
